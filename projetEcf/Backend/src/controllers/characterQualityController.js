...
export const characterQualityController = {
    ...
    getAllCharacterQualities: async (res) => {
        try {
            const characterQualities = await characterQualityService.getAll();
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ success: true, data: characterQualities }));
        } catch (error) {
            logError(error);
    ...
    getCharacterQualitiesByCharacterId: async (res, id) => {
        try {
    ...
    getCharacterQualityById: async (res, idcharacter, idquality) => {
        try {
            const characterQuality = await characterQualityService.get(idcharacter, idquality);
            if (characterQuality) {
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: characterQuality }));
            } else {
                res.writeHead(404, { 'Content-Type': 'application/json' });
    ...
    deleteCharacterQuality: async (res, idcharacter, idquality) => {
        try {
